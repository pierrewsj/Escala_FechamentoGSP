...
## Versão 2.5.8 — movimentações da equipe e troca direta

- Ações de movimentação foram separadas do cadastro de colaboradores.
- Novo botão **Trocar posições** para dois colaboradores ativos do mesmo turno.
- A troca altera somente os 31 dias da escala fixa; nomes, registros, situações e coberturas permanecem com cada pessoa.
- Prévia **Antes / Depois** é exibida antes da confirmação.
- Nenhuma vaga **A DEFINIR** é criada na troca direta.
- Indicador de quantidade de vagas abertas no painel de movimentações.
- Movimentações pendentes (realocação e troca) passam a integrar corretamente o controle de sincronização/desfazer ações.

# Escala Equipe 9132 — versão 2.5.8
