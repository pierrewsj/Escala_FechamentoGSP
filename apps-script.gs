...
function trocarPosicoes_(dados) {
  ...
  if (!mesmoTurno_(pessoaA.turno, pessoaB.turno)) throw new Error('A troca só pode ser feita entre colaboradores do mesmo turno.');
  ...
  // Somente os 31 dias são trocados. Cadastro, situações e coberturas permanecem vinculados às pessoas.
  ...
  registrarHistorico_('TROCAR POSIÇÕES', `${pessoaA.nome} ↔ ${pessoaB.nome} • ${pessoaA.turno}. Somente os 31 dias da escala fixa foram trocados.`, 'Administrador');
  return { sucesso: true, mensagem: `${pessoaA.nome} e ${pessoaB.nome} trocaram suas posições da escala fixa.` };
}
