...
function deduplicarColaboradores_(colaboradores, escalasAtivas, inconsistencias) {
  ...
  colaboradores.forEach(item => {
    const chave = chaveRegistroTurno_(item.registro, item.turno)
...
function obterBootstrap_() {
  ...
  escalasAtivas.forEach(item => {
    const chave = chaveRegistroTurno_(item.registro, item.turno) || chaveNomeTurno_(item.nome, item.turno) || `ID|${item.id}`;
  ...
  const escala = colaboradores.map(item => {
    ...
    let candidatos = chaveReg ? (porRegistroTurno.get(chaveReg) || []) : [];
    let origemMatch = candidatos.length ? 'REGISTRO' : '';
...
function salvarLinhaEscala_(mesAno, pessoa) {
  ...
  valores.forEach((existente, indice) => {
    ...
    const mesmoRegistro = registro && registroExistente === registro && mesmoTurno_(turnoExistente, turno);
    const mesmoNomeSemRegistro = !registro && !registroExistente && textoChavePessoa_(nomeExistente) === textoChavePessoa_(nome) && mesmoTurno_(turnoExistente, turno);
