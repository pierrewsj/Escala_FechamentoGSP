...
const CABECALHOS = Object.freeze({
  ...
  coberturas: ['ID', 'ID Origem', 'Nome Origem', 'ID Destino', 'Nome Destino', 'Data Inicial', 'Data Final', 'Ativo', 'Criado Em', 'Atualizado Em'],
...
function garantirEstruturaSeNecessario_() {
  ...
  const pronta = nomesObrigatorios.every((nome) => Boolean(planilha.getSheetByName(nome)));
  if (!pronta) garantirEstrutura_();
}
...
function obterBootstrap_() {
  ...
  // Colaboradores cadastrados diretamente na aba Colaboradores também aparecem,
  // mesmo que ainda não possuam uma linha na aba Escala.
  const escala = colaboradores.map(item => {
    const existente = mapaEscala.get(item.id);
    if (existente) {
      return {
        ...existente,
        nome: item.nome || existente.nome,
        registro: item.registro || existente.registro,
        turno: turnoCanonico_(item.turno || existente.turno),
        ativo: true
      };
    }
    return {
      ...
      turno: turnoCanonico_(item.turno || '1º Turno'),
      dias: Array(31).fill(''),
      ativo: true
    };
  });
...
function lerCoberturas_() {
  ...
  return obterValoresSemCabecalho_(aba, CABECALHOS.coberturas.length)
    ...
    .filter(item => item.id && item.origemId && item.destinoId && item.inicio && item.fim && item.ativo);
}
...
function salvarCobertura_(dados) {
  ...
  const inicio = normalizarDataISO_(item.inicio || item.dataInicial);
  const fim = normalizarDataISO_(item.fim || item.dataFinal);
  if (!origemId || !destinoId || origemId === destinoId || !inicio || !fim) throw new Error('Dados da cobertura incompletos.');
  if (fim < inicio) throw new Error('A data final não pode ser anterior à data inicial.');
  const colaboradores = lerColaboradoresAtivos_();
  const origem = colaboradores.find(p => p.id === origemId);
  const destino = colaboradores.find(p => p.id === destinoId);
  if (!origem || !destino) throw new Error('Colaborador da cobertura não encontrado.');
  const agora = new Date();
  const id = String(item.id || Utilities.getUuid());
  const linha = [id, origemId, origem.nome, destinoId, destino.nome, inicio, fim, true, item.criadoEm ? new Date(item.criadoEm) : agora, agora];
  upsertPorChave_(NOMES_ABAS.coberturas, CABECALHOS.coberturas.length, 1, id, linha);
  registrarHistorico_('SALVAR COBERTURA', `${destino.nome} cobre ${origem.nome} • ${inicio} a ${fim}.`, 'Administrador');
