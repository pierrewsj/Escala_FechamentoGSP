...
function fecharModalSelecaoApp() {
  ...
  if (!haOutroModalAberto('modalSelecaoApp')) document.body.classList.remove('modal-aberto');
  setTimeout(() => {
    if (foco && document.contains(foco) && typeof foco.focus === 'function') foco.focus({ preventScroll: true });
  }, 0);
}

function instalarSelectsEmModal() {
  // Captura antes do navegador abrir o seletor nativo (Android/iOS/desktop).
  document.addEventListener('pointerdown', (evento) => {
    const select = evento.target?.closest?.('select');
    if (!select || select.disabled) return;
    if (select.closest('#modalSelecaoApp')) return;
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.6.0'}`;
