...
function resolverConfirmacaoApp(resultado) {
  ...
  if ($('modalCrudAdmin')?.hidden !== false && $('modalSituacaoEscala')?.hidden !== false) document.body.classList.remove('modal-aberto');
...
async function salvarPostoPeloModal() {
  ...
  if (codigoOriginal && codigoOriginal !== codigo) {
    ...
    ESCALA.forEach((pessoa) => { pessoa.dias = pessoa.dias.map((valorDia) => { const partes = decomporDiaEscala(valorDia); return comporDiaEscala(partes.posto === codigoOriginal ? codigo : partes.posto, partes.situacao); }); });
  }
...
function resumoDiaEscala(valor) {
  const { posto, situacao } = decomporDiaEscala(valor);
  return {
    posto,
    situacao,
    descricaoPosto: posto ? postoDescricao(posto) : 'Sem posto definido',
...
function recuperarPostosDeSituacoesLegadas() {
  let recuperados = 0;
  const restaurarDiasVazios = new Set(['ADRIANA', 'JUNIOR']);
  ESCALA.forEach((pessoa) => {
    ...
    pessoa.dias = Array.from({ length: 31 }, (_, indice) => {
      ...
      const deveRecuperar = Boolean(partes.situacao) || restaurarDiasVazios.has(nomeReferencia);
...
function escalaSabadoConfereComPublicacao(escalaServidor, escalaLocal) {
  if (!escalaServidor?.encontrada || escalaServidor.status !== 'PUBLICADA') return false;
...
async function verificarPublicacaoSabadoNoServidor(escalaLocal, { tentativas = 6 } = {}) {
  let ultimaResposta = null;
  for (let tentativa = 1; tentativa <= tentativas; tentativa += 1) {
    try {
      const resposta = await apiGet('escalaSabado', {
        equipe: APP.equipe || CONFIG.equipe,
        data: escalaLocal.dataISO,
        turno: escalaLocal.turno,
        incluirRascunho: ''
      });
      ultimaResposta = normalizarEscalaSabado(resposta, escalaLocal.dataISO, escalaLocal.turno);
      if (escalaSabadoConfereComPublicacao(ultimaResposta, escalaLocal)) {
...
function garantirPayloadNaFila(payload) {
  const fila = lerFila();
  const jaExiste = fila.some((item) => item?.payload?.acao === payload.acao
...
async function processarFilaOffline() {
  ...
  for (const item of fila) {
    try {
      ...
      if (item?.payload?.acao === 'salvarEscalaSabado' && String(item.payload.status || '').toUpperCase() === 'PUBLICADA') {
        const escalaFila = normalizarEscalaSabado(item.payload, item.payload.dataISO, item.payload.turno);
        const verificacao = await verificarPublicacaoSabadoNoServidor(escalaFila, { tentativas: 3 });
        if (!verificacao.ok) {
          restantes.push(item);
          continue;
        }
  ...
  $('resultadoConteudo').innerHTML = `
    <div class="lista-cards">
      ...
      ${pessoa.dias.slice(0, limite).map((valorDia, indice) => {
        const diaEscala = resumoDiaEscala(valorDia);
        return card({
          titulo: `Dia ${String(indice + 1).padStart(2, '0')}`,
          badge: diaEscala.posto || '—',
          descricao: diaEscala.situacao && diaEscala.posto ? `Posto de referência: ${diaEscala.posto} — ${diaEscala.descricaoPosto}` : diaEscala.descricaoPosto,
          subtitulo: diaEscala.descricaoSituacao,
  ...
  $('resultadoConteudo').innerHTML = `
    <div class="lista-cards">
      ...
      ${pessoas.length ? pessoas.map((pessoa) => {
        const diaEscala = resumoDiaEscala(pessoa.dias[dia - 1] || '');
        const subtitulo = `${pessoa.turno}${diaEscala.descricaoSituacao ? ` • ${diaEscala.descricaoSituacao}` : ''}`;
        const descricao = diaEscala.situacao && diaEscala.posto ? `Posto de referência: ${diaEscala.posto} — ${diaEscala.descricaoPosto}` : diaEscala.descricaoPosto;
        return card({ titulo: pessoa.nome, badge: diaEscala.posto || '—', descricao, subtitulo, hoje: dia === destaque, situacao: Boolean(diaEscala.situacao) });
...
function buscarPorPosto(postoBusca) {
  ...
  pessoasPorTurno(turno).forEach((pessoa) => {
    pessoa.dias.slice(0, diasNoMes()).forEach((valorDia, indice) => {
      const diaEscala = resumoDiaEscala(valorDia);
      if (diaEscala.posto === postoBusca) itens.push(card({
        titulo: pessoa.nome,
        badge: `Dia ${String(indice + 1).padStart(2, '0')}`,
        descricao: `${postoBusca} • ${postoDescricao(postoBusca)}`,
        subtitulo: `${pessoa.turno}${diaEscala.descricaoSituacao ? ` • ${diaEscala.descricaoSituacao}` : ''}`,
        hoje: indice + 1 === destaque,
        situacao: Boolean(diaEscala.situacao)
      }));
    });
...
async function preencherDadosCargaPorColaborador(resetarSetorManual = false) {
  ...
  let posto = postoDoDia(valorDiaCarga);
  let origemPosto = situacaoDoDia(valorDiaCarga) ? ` • ${descricaoSituacao(situacaoDoDia(valorDiaCarga))}` : '';
...
function atualizarDestaqueSituacaoDia(dia, valorDia = '') {
  const bloco = $(`diaAdminBloco_${dia}`);
  const etiqueta = $(`situacaoDia_${dia}`);
  const partes = decomporDiaEscala(valorDia);
  const ehSituacao = Boolean(partes.situacao);
  bloco?.classList.toggle('situacao-aplicada', ehSituacao);
  if (etiqueta) {
    etiqueta.hidden = !ehSituacao;
    etiqueta.textContent = ehSituacao
      ? `${descricaoSituacao(partes.situacao)} • Posto de referência: ${partes.posto || 'não definido'}`
      : '';
  }
}
...
function aoAlterarPostoDia(dia) {
  ...
  const valorAtual = pessoa?.dias?.[dia - 1] || '';
  const postoSelecionado = $(`diaAdmin_${dia}`)?.value || '';
  atualizarDestaqueSituacaoDia(dia, comporDiaEscala(postoSelecionado, situacaoDoDia(valorAtual)));
}
...
async function salvarPosto() {
  ...
  if (codigoOriginal && codigoOriginal !== codigo) {
    ...
    ESCALA.forEach((pessoa) => {
      pessoa.dias = pessoa.dias.map((valorDia) => { const partes = decomporDiaEscala(valorDia); return comporDiaEscala(partes.posto === codigoAnterior ? codigo : partes.posto, partes.situacao); });
    });
  }
  LEGENDA[codigo] = { local, setor: setor || local, ativo: true };
  salvarCacheMes();
...
async function excluirPosto() {
  ...
  delete LEGENDA[codigo];
  ESCALA.forEach((pessoa) => {
    pessoa.dias = pessoa.dias.map((valorDia) => { const partes = decomporDiaEscala(valorDia); return comporDiaEscala(partes.posto === codigo ? '' : partes.posto, partes.situacao); });
  });
  salvarCacheMes();
  atualizarInterfaceCompleta({ preservarAdmin: true });
...
function renderizarSituacoesEscala() {
  ...
  campo.innerHTML = opcoes.map((item) => `
    <button type="button" class="situacao-opcao ${editorSituacaoEscala.codigo === item.codigo ? 'selecionado' : ''}" aria-pressed="${editorSituacaoEscala.codigo === item.codigo}" onclick="selecionarSituacaoEscala('${escaparHTML(item.codigo)}')">
      <span class="radio-modal" aria-hidden="true"></span>
...
async function selecionarSituacaoEscala(codigo) {
  ...
  try {
    ...
    const periodo = inicioISO === fimISO
    ...
    const confirmado = await confirmarAcaoApp({
      ...
      mensagem: `Aplicar ${nomeSituacao} para ${pessoa.nome}?\n\nPeríodo: ${periodo}\nO posto de trabalho será mantido e o nome ficará destacado em amarelo.`,
    ...
    await aplicarSituacaoEscala({ fecharAoConcluir: true });
  } catch (erro) {
    editorSituacaoEscala.codigo = '';
    renderizarSituacoesEscala();
...
async function aplicarSituacaoEscala({ fecharAoConcluir = true } = {}) {
  ...
  for (let dia = diaInicio; dia <= diaFim; dia += 1) {
    const atual = decomporDiaEscala(pessoa.dias[dia - 1]);
    const postoPreservado = atual.posto || postoReferenciaPadrao(pessoa, dia - 1);
    pessoa.dias[dia - 1] = comporDiaEscala(postoPreservado, codigo);
  }
  salvarCacheMes();
  atualizarInterfaceCompleta({ preservarAdmin: true });
  ...
  toast(`${descricao} aplicada para ${pessoa.nome}.`, 'ok');
...
async function removerSituacaoEscala() {
  ...
  try {
    ...
    for (let dia = diaInicio; dia <= diaFim; dia += 1) {
      ...
      if (atual.situacao) {
        const postoPreservado = atual.posto || postoReferenciaPadrao(pessoa, dia - 1);
        pessoa.dias[dia - 1] = comporDiaEscala(postoPreservado, '');
        removidos += 1;
      }
    }
...
async function salvarEscala() {
  ...
  pessoa.dias = Array.from({ length: 31 }, (_, indice) => {
    ...
    return comporDiaEscala(postoSelecionado, situacaoDoDia(valorAtual));
  });
  salvarCacheMes();
  analisarConflitos();
  atualizarInterfaceCompleta({ preservarAdmin: true });
...
function analisarConflitos() {
  ...
  pessoasAtivas().forEach((pessoa) => {
    pessoa.dias.slice(0, diasNoMes()).forEach((valorDia, indice) => {
      const partes = decomporDiaEscala(valorDia);
      const posto = partes.posto;
      if (!posto || partes.situacao || ['Pens.', 'Ilha'].includes(posto)) return;
...
function renderizarEditorEscalaSabado(escala = null) {
  ...
  lista.innerHTML = pessoas.length ? pessoas.map((pessoa, indice) => {
    ...
    const valorDiaFixo = pessoa.dias[diaDoMesDaData($('dataEscalaSabado')?.value || '') - 1] || '';
    const postoFixo = postoDoDia(valorDiaFixo);
    const situacaoFixa = situacaoDoDia(valorDiaFixo);
    const sugestaoFixa = postoFixo ? ` • Fixo: ${postoFixo}${situacaoFixa ? ` • ${descricaoSituacao(situacaoFixa)}` : ''}` : '';
  ...
  try {
    ...
    if (statusFinal !== 'PUBLICADA') {
      ...
      if (envio?.enfileirado) {
        ...
        msgAdmin(`Rascunho de sábado ${formatarDataBR(dataISO)} salvo.`, 'ok');
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.4.7'}`;
