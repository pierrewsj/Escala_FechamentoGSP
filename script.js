...
function garantirPayloadAdminNaFila(payload) {
  ...
  const chave = JSON.stringify({
    acao: payload?.acao,
    id: payload?.situacao?.id || payload?.cobertura?.id || payload?.vagaId || payload?.colaborador?.id || payload?.colaboradorId || payload?.codigo || payload?.posto?.codigo || '',
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.5.5'}`;
