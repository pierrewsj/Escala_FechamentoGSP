...
function obterCacheEscalaSabado(dataISO, turno = 'todos') {
  ...
  const escalasDaData = Object.values(cache).filter((item) => item?.dataISO === dataISO);
  if (turno !== 'todos') {
...
function renderizarEscalaSabadoPublica(dataISO, turnoSelecionado, escala, { atualizando = false } = {}) {
  const itens = (escala?.itens || []).filter((item) =>
    item.status === 'PUBLICADA' && (turnoSelecionado === 'todos' || mesmoTurno(item.turno, turnoSelecionado))
  );
...
async function mostrarProximoSabado(botao = null) {
  abrirAba('resultado', botao || document.querySelector('.aba-sabado'));
  const turnoSelecionado = $('selectTurnoConsulta')?.value || 'todos';
  const inicio = dataISOHoje();

  // Mostra imediatamente uma escala publicada já salva no aparelho.
  const datasCache = Object.values(lerCacheSabados())
    .filter((item) => item?.status === 'PUBLICADA' && item?.dataISO >= inicio)
    .filter((item) => turnoSelecionado === 'todos' || mesmoTurno(item.turno, turnoSelecionado))
    .sort((a, b) => String(a.dataISO).localeCompare(String(b.dataISO)));
  if (datasCache[0]?.itens?.length) {
    renderizarEscalaSabadoPublica(datasCache[0].dataISO, turnoSelecionado, datasCache[0], { atualizando: true });
  } else {
    renderizarEscalaSabadoPublica(proximoSabadoISO(true), turnoSelecionado, normalizarEscalaSabado({}, proximoSabadoISO(true), turnoSelecionado), { atualizando: true });
  }

  try {
    const resposta = await apiGet('proximaEscalaSabado', {
      equipe: APP.equipe || CONFIG.equipe,
      data: inicio,
      turno: turnoSelecionado === 'todos' ? '' : turnoSelecionado
    });
    const dataEncontrada = resposta?.dataISO || proximoSabadoISO(true);
    const escala = normalizarEscalaSabado(resposta, dataEncontrada, turnoSelecionado);
    if (escala.encontrada && escala.status === 'PUBLICADA' && escala.itens.length) {
      salvarCacheEscalaSabado(escala);
      renderizarEscalaSabadoPublica(dataEncontrada, turnoSelecionado, escala);
      return;
    }
    if (datasCache[0]?.itens?.length) {
      renderizarEscalaSabadoPublica(datasCache[0].dataISO, turnoSelecionado, datasCache[0]);
      return;
    }
    renderizarEscalaSabadoPublica(dataEncontrada, turnoSelecionado, escala);
  } catch (erro) {
    if (datasCache[0]?.itens?.length) {
      ...
      const dataPadrao = proximoSabadoISO(true);
      renderizarEscalaSabadoPublica(dataPadrao, turnoSelecionado, normalizarEscalaSabado({}, dataPadrao, turnoSelecionado));
    }
...
async function salvarEscalaSabado(status) {
  ...
  try {
    ...
    if (envio.enfileirado) {
      msgAdmin('Escala salva no aparelho e aguardando conexão com a planilha.', 'aviso');
      toast('Escala aguardando conexão para ser enviada.', 'aviso');
    } else {
      // O POST do Apps Script é opaco no navegador. Confirma a gravação por uma leitura logo depois.
      await new Promise((resolve) => setTimeout(resolve, 900));
      const confirmacao = await carregarEscalaSabado(dataISO, turno, {
        forcar: true,
        incluirRascunho: true,
        silencioso: true
      });
      const confirmado = confirmacao?.itens?.length > 0
        && confirmacao.status === statusFinal
        && confirmacao.itens.length === itens.length;

      if (confirmado) {
        escalaSabadoEditor = confirmacao;
        salvarCacheEscalaSabado(confirmacao);
        msgAdmin(statusFinal === 'PUBLICADA'
          ? `Escala de sábado ${formatarDataBR(dataISO)} publicada e confirmada na planilha.`
          : `Rascunho de sábado ${formatarDataBR(dataISO)} salvo e confirmado na planilha.`, 'ok');
        toast(statusFinal === 'PUBLICADA' ? 'Escala publicada e confirmada.' : 'Rascunho salvo e confirmado.', 'ok');
      } else {
        msgAdmin('O envio foi realizado, mas a planilha ainda não confirmou a gravação. Aguarde alguns segundos e use Atualizar.', 'aviso');
        toast('Envio realizado; confirmação da planilha pendente.', 'aviso');
      }
    }
  } catch (erro) {
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.3.6'}`;
