...
function abrirTrocaPosicoes() {
  ...
  $('subtituloModalCrudAdmin').textContent = 'Use quando dois colaboradores ativos do mesmo turno permanecerem na equipe, mas precisarem trocar suas posições completas da escala fixa.';
  modal.hidden = false;
  document.body.classList.add('modal-aberto');
...
function renderizarFormularioTrocarPosicoes(primeiroId = '', segundoId = '') {
  ...
  const candidatosSegundo = colaboradores.filter((item) => item.id !== primeiro.id && mesmoTurno(item.turno, primeiro.turno));
  ...
  conteudo.innerHTML = `
    <div class="form-preencher-vaga">
      ...
      <div class="modal-campo-bloco">
        ...
        <select id="selectSegundoTrocaPosicoes">${candidatosSegundo.length ? candidatosSegundo.map((item) => `<option value="${escaparHTML(item.id)}" ${segundo?.id === item.id ? 'selected' : ''}>${escaparHTML(item.nome)} • ${escaparHTML(item.turno)}${item.registro ? ` • Reg. ${escaparHTML(item.registro)}` : ''}</option>`).join('') : '<option value="">Nenhum outro colaborador com escala neste turno</option>'}</select>
        <small class="muted mini-ajuda">Somente colaboradores do mesmo turno são exibidos. Nenhuma vaga A DEFINIR é criada nesta operação.</small>
      </div>
      ${segundo ? `
        <div class="preview-troca-posicoes">
          ...
          <div class="aviso-troca-vinculos">A troca altera somente os 31 postos da escala fixa. Nome, registro, situações (férias, atestado etc.) e coberturas continuam vinculados à própria pessoa.${possuiVinculos ? ` Há registros temporários futuros/ativos: ${escaparHTML(primeiro.nome)} (${vinculosPrimeiro.situacoes} situação(ões), ${vinculosPrimeiro.coberturas} cobertura(s)); ${escaparHTML(segundo.nome)} (${vinculosSegundo.situacoes} situação(ões), ${vinculosSegundo.coberturas} cobertura(s)).` : ''}</div>
        </div>` : '<div class="modal-vazio">Não existe outro colaborador com escala fixa no mesmo turno para realizar a troca.</div>'}
    </div>`;
...
async function trocarPosicoesPeloModal() {
  ...
  if (!mesmoTurno(primeiro.turno, segundo.turno)) { toast('A troca só pode ser feita entre colaboradores do mesmo turno.', 'erro'); return; }
  ...
  const confirmado = await confirmarAcaoApp({
    ...
    mensagem: `${primeiro.nome} e ${segundo.nome} trocarão suas posições completas da escala fixa (${primeiro.turno}).\n\nANTES\n${primeiro.nome}: ${resumoPosicaoParaTroca(primeiro)}\n${segundo.nome}: ${resumoPosicaoParaTroca(segundo)}\n\nDEPOIS\n${primeiro.nome}: posição de ${segundo.nome}\n${segundo.nome}: posição de ${primeiro.nome}\n\nNenhum colaborador será removido e nenhuma vaga A DEFINIR será criada. Situações e coberturas continuam vinculadas à própria pessoa.`,
  ...
  try {
    ...
    msgAdmin(`${primeiro.nome} e ${segundo.nome} trocaram suas posições da escala fixa.`);
...
function payloadAdminConfirmadoNoBootstrap(payload, resposta) {
  ...
  if (acao === 'trocarPosicoes') {
    ...
    return esperadoA.length === recebidoA.length && esperadoB.length === recebidoB.length
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.5.8'}`;
