...
const ACOES_ADMIN_DADOS = new Set([
  'salvarColaborador', 'excluirColaborador', 'salvarPosto', 'excluirPosto',
...
function deduplicarEscalaLocal(lista) {
  ...
  // Se existir uma cópia sem registro e outra com registro do mesmo nome/turno,
  // mantém apenas a versão identificada por registro, preservando os postos mais completos.
  ...
  $('resultadoConteudo').innerHTML = `
    <div class="lista-cards">
      ...
      ${pessoas.length ? pessoas.map((pessoa) => {
        ...
        const subtitulo = [pessoa.turno, diaEscala.descricaoSituacao, diaEscala.cobrindoTexto, diaEscala.coberturaNome ? `Cobertura: ${diaEscala.coberturaNome}` : ''].filter(Boolean).join(' • ');
        const descricao = diaEscala.cobrindo
          ? diaEscala.origemCoberturaTexto
          : (diaEscala.situacao && diaEscala.posto ? `Posto de referência: ${diaEscala.posto} — ${diaEscala.descricaoPosto}` : diaEscala.descricaoPosto);
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.5.3'}`;
