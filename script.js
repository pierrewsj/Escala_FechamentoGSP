...
const ACOES_ADMIN_DADOS = new Set([
  'salvarColaborador', 'excluirColaborador', 'retirarColaborador', 'preencherVaga', 'realocarColaborador', 'trocarPosicoes', 'salvarPosto', 'excluirPosto',
...
function turnosDisponiveis() {
  // A equipe 9132 opera somente com 1º e 2º turno. O 3º turno era um resíduo
  // de configuração antiga e podia aparecer indevidamente nos seletores.
  const configurados = Array.isArray(CONFIG.turnos) ? CONFIG.turnos : ['1º Turno', '2º Turno'];
  const permitidos = configurados.filter((turno) => ['1 TURNO', '2 TURNO'].includes(normalizarTurnoComparacao(turno)));
  return permitidos.length ? [...new Set(permitidos.map((turno) => normalizarTurnoComparacao(turno) === '2 TURNO' ? '2º Turno' : '1º Turno'))] : ['1º Turno', '2º Turno'];
}
...
function abrirSelectComoModal(select) {
  ...
  if (busca) {
    busca.value = '';
    const qtdAtivas = opcoesSelectModal(select).filter((item) => !item.desabilitada).length;
    busca.hidden = qtdAtivas <= 7;
    busca.placeholder = `Pesquisar em ${qtdAtivas} opção${qtdAtivas === 1 ? '' : 'ões'}...`;
  }
  ...
  setTimeout(() => {
    ...
    (buscaVisivel ? busca : modal.querySelector('.modal-selecao-opcao.selecionada, .modal-selecao-opcao:not(:disabled)'))?.focus();
  }, 20);
}
...
function interceptarSelectNativo(evento) {
  const select = evento.target?.closest?.('select');
  if (!select || select.disabled || select.closest('#modalSelecaoApp')) return;

  // Impede completamente o seletor nativo do Android/iOS/desktop. Na 2.6.0
  // apenas pointerdown era bloqueado; alguns navegadores ainda executavam a
  // ação padrão no click/touch e podiam alterar o valor para outra opção.
...
function instalarSelectsEmModal() {
  // Cobertura redundante proposital: diferentes navegadores acionam <select>
  // por sequências diferentes de eventos. Todos são capturados antes do nativo.
  document.addEventListener('pointerdown', interceptarSelectNativo, true);
  document.addEventListener('mousedown', interceptarSelectNativo, true);
  document.addEventListener('touchstart', interceptarSelectNativo, { capture: true, passive: false });
  document.addEventListener('click', interceptarSelectNativo, true);
...
function colaboradoresElegiveisTroca() {
  return pessoasAtivas()
    ...
    .slice()
    .sort((a, b) => `${a.turno}|${a.nome}|${a.registro || ''}`.localeCompare(`${b.turno}|${b.nome}|${b.registro || ''}`, 'pt-BR'));
}
...
function renderizarFormularioTrocarPosicoes(primeiroId = '', segundoId = '') {
  ...
  if (colaboradores.length < 2) {
    conteudo.innerHTML = '<div class="modal-vazio">São necessários pelo menos dois colaboradores ativos com escala fixa para realizar uma troca de posições.</div>';
    acoes.innerHTML = '<button type="button" class="btn secundario" onclick="fecharModalCrudAdmin()">Fechar</button>';
    return;
  }

  const primeiro = colaboradores.find((item) => item.id === primeiroId) || colaboradores[0];
  const candidatosSegundo = colaboradores.filter((item) => item.id !== primeiro.id);
  const segundo = candidatosSegundo.find((item) => item.id === segundoId) || candidatosSegundo[0] || null;
  ...
  conteudo.innerHTML = `
    <div class="form-preencher-vaga">
      <div class="modal-campo-bloco">
        ...
        <select id="selectPrimeiroTrocaPosicoes">${colaboradores.map((item) => `<option value="${escaparHTML(item.id)}" ${item.id === primeiro.id ? 'selected' : ''}>${escaparHTML(item.nome)} • ${escaparHTML(item.turno)}${item.registro ? ` • Reg. ${escaparHTML(item.registro)}` : ''}</option>`).join('')}</select>
      </div>
      <div class="modal-campo-bloco">
        <label for="selectSegundoTrocaPosicoes">2. Segundo colaborador</label>
        <select id="selectSegundoTrocaPosicoes">${candidatosSegundo.length ? candidatosSegundo.map((item) => `<option value="${escaparHTML(item.id)}" ${segundo?.id === item.id ? 'selected' : ''}>${escaparHTML(item.nome)} • ${escaparHTML(item.turno)}${item.registro ? ` • Reg. ${escaparHTML(item.registro)}` : ''}</option>`).join('') : '<option value="">Nenhum outro colaborador com escala disponível</option>'}</select>
        <small class="muted mini-ajuda">Colaboradores dos dois turnos são exibidos. Se os turnos forem diferentes, cada pessoa também assumirá o turno da posição recebida. Nenhuma vaga A DEFINIR é criada.</small>
      </div>
      ${segundo ? `
  ...
  $('selectPrimeiroTrocaPosicoes')?.addEventListener('change', (evento) => renderizarFormularioTrocarPosicoes(evento.target.value, ''));
  $('selectSegundoTrocaPosicoes')?.addEventListener('change', (evento) => renderizarFormularioTrocarPosicoes(primeiro.id, evento.target.value));
  acoes.innerHTML = `
...
async function trocarPosicoesPeloModal() {
  ...
  if (!primeiro || !segundo || ehVagaAberta(primeiro) || ehVagaAberta(segundo)) { toast('Selecione dois colaboradores válidos.', 'erro'); return; }
  if (primeiro.id === segundo.id) { toast('Escolha dois colaboradores diferentes.', 'erro'); return; }
...
function pessoasPorTurno(turno = 'todos') {
  const pessoas = pessoasEscalaAtivas();
...
async function processarFilaOffline() {
  ...
  for (const item of fila) {
    try {
      ...
      if (['salvarColaborador','excluirColaborador','retirarColaborador','preencherVaga','realocarColaborador','trocarPosicoes','salvarPosto','excluirPosto','salvarEscala','salvarSituacao','removerSituacao','salvarCobertura','removerCobertura'].includes(String(item?.payload?.acao || ''))) {
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.6.1'}`;
