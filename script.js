...
const GOOGLE_SHEETS_URL = 'https://script.google.com/macros/s/AKfycbyRZBM1GWQoaKao4QrIzghBu9w88KtFfGG_jd56a4vB9Hx9qyPupqzayKq6mfajbLEkmg/exec';
...
async function exportarWhatsappLancamentoCarga() {
  const dados = dadosLancamentoCargaAtual('exportar para o WhatsApp');
  if (!dados) return;

  const texto = `FECHAMENTO DE CARGAS\n\nData: ${dados.data}\nColaborador: ${dados.colaborador}\nRegistro: ${dados.registro}\nTurno: ${dados.turno}\nSetor/Posto: ${dados.setorPosto}\nQuantidade de cargas liberadas: ${dados.quantidade}`;
  ...
  try {
    ...
    msgCargas('Dados enviados para a planilha e mensagem preparada no WhatsApp.');
  } catch (erro) {
    console.error(erro);
    msgCargas('Não foi possível enviar para a planilha. Verifique a internet e tente novamente.', 'erro');
  } finally {
    if (botao) {
      botao.disabled = false;
      botao.textContent = 'Exportar WhatsApp';
    }
  }
}
...
Object.assign(window, {
  ...
  exportarWhatsappCargas,
  exportarWhatsappLancamentoCarga,
