...
async function enviarLancamentoGoogleSheets() {
  const dados = dadosLancamentoCargaAtual('enviar para a planilha');
  if (!dados) return;
  ...
  try {
    ...
    if (botao) {
      ...
      botao.textContent = 'Enviar para Planilha';
    }
  }
}
...
function exportarWhatsappLancamentoCarga() {
  const texto = gerarTextoWhatsappLancamentoCarga();
  if (!texto) return;
  const url = `https://wa.me/?text=${encodeURIComponent(texto)}`;
  window.open(url, '_blank');
  msgCargas('Lançamento preparado para envio no WhatsApp.');
}
