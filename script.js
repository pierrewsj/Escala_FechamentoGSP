...
function normalizarRegistrosTemporais() {
  ...
  COBERTURAS_REGISTROS = (Array.isArray(COBERTURAS_REGISTROS) ? COBERTURAS_REGISTROS : [])
    ...
    .filter(item => item.id && item.origemId && item.destinoId && /^\d{4}-\d{2}-\d{2}$/.test(item.inicio) && /^\d{4}-\d{2}-\d{2}$/.test(item.fim) && item.ativo)
    .sort((a, b) => a.inicio.localeCompare(b.inicio));
}
...
function resolverConfirmacaoApp(resultado) {
  ...
  if ($('modalCrudAdmin')?.hidden !== false && $('modalSituacaoEscala')?.hidden !== false && $('modalPreviewSabado')?.hidden !== false) document.body.classList.remove('modal-aberto');
...
function resumoDiaEscala(valor, colaboradorId = '', dataISO = '') {
  ...
  const coberturaDestino = cobertura?.destinoId ? colaboradorPorId(cobertura.destinoId) : null;
  return {
    posto: partes.posto,
    situacao,
    situacaoRegistro: registroSituacao,
    descricaoPosto: partes.posto ? postoDescricao(partes.posto) : 'Sem posto definido',
    descricaoSituacao: situacao ? descricaoSituacao(situacao) : '',
    cobertura,
    coberturaNome: coberturaDestino?.nome || cobertura?.nomeDestino || ''
  };
}
...
function normalizarDadosAtuais() {
  ...
  // A partir da 2.5.0, a escala fixa guarda somente o posto. Situações têm datas reais e ficam separadas.
  ESCALA.forEach((pessoa) => {
...
async function processarFilaOffline() {
  ...
  for (const item of fila) {
    try {
      ...
      if (['salvarColaborador','excluirColaborador','salvarPosto','excluirPosto','salvarEscala','salvarSituacao','removerSituacao','salvarCobertura','removerCobertura'].includes(String(item?.payload?.acao || ''))) {
  ...
  $('resultadoConteudo').innerHTML = `
    <div class="lista-cards">
      ...
      ${pessoas.length ? pessoas.map((pessoa) => {
        ...
        const subtitulo = `${pessoa.turno}${diaEscala.descricaoSituacao ? ` • ${diaEscala.descricaoSituacao}` : ''}${diaEscala.coberturaNome ? ` • Cobertura: ${diaEscala.coberturaNome}` : ''}`;
  ...
  pessoasPorTurno(turno).forEach((pessoa) => {
    pessoa.dias.slice(0, diasNoMes()).forEach((valorDia, indice) => {
      ...
      if (diaEscala.posto === postoBusca) itens.push(card({
        ...
        subtitulo: `${pessoa.turno}${diaEscala.descricaoSituacao ? ` • ${diaEscala.descricaoSituacao}` : ''}${diaEscala.coberturaNome ? ` • Cobertura: ${diaEscala.coberturaNome}` : ''}`,
        hoje: indice + 1 === destaque,
        situacao: Boolean(diaEscala.situacao)
      }));
    });
...
async function preencherDadosCargaPorColaborador(resetarSetorManual = false) {
  ...
  const situacaoCarga = situacaoNaData(pessoa.id, dataCarga);
  const coberturaCarga = coberturaNaDataPorOrigem(pessoa.id, dataCarga);
  let origemPosto = situacaoCarga ? ` • ${descricaoSituacao(situacaoCarga.tipo)}` : '';
  if (coberturaCarga) origemPosto += ` • Cobertura: ${colaboradorPorId(coberturaCarga.destinoId)?.nome || coberturaCarga.nomeDestino || 'definida'}`;
...
function preencherSelectsAdmin() {
  ...
  if ($('adminColaborador')) $('adminColaborador').innerHTML = opcoes || '<option value="">Nenhum colaborador</option>';
  if ($('coberturaOrigem')) $('coberturaOrigem').innerHTML = '<option value="">Selecione quem será coberto</option>' + opcoes;
  if ($('coberturaDestino')) $('coberturaDestino').innerHTML = '<option value="">Selecione o substituto</option>' + opcoes;
...
function abrirModalSituacaoEscala() {
  if (!exigirAdmin()) return;
  const pessoas = pessoasAtivas().slice().sort((a, b) => `${a.turno}${a.nome}`.localeCompare(`${b.turno}${b.nome}`, 'pt-BR'));
  editorSituacaoEscala = { colaboradorId: $('adminColaborador')?.value || pessoas[0]?.id || '', codigo: '' };
...
async function salvarEscala() {
  ...
  const pessoa = colaboradorPorId($('adminColaborador')?.value || '');
  if (!pessoa) return;
  pessoa.dias = diasDisponiveis().map((dia) => normalizarCodigoPosto($(`diaAdmin_${dia}`)?.value || ''));
  salvarCacheMes();
  atualizarInterfaceCompleta({ preservarAdmin: true });
...
function garantirPayloadAdminNaFila(payload) {
  ...
  const chave = JSON.stringify({
    acao: payload?.acao,
    id: payload?.situacao?.id || payload?.cobertura?.id || payload?.colaborador?.id || payload?.colaboradorId || payload?.codigo || payload?.posto?.codigo || '',
    inicio: payload?.situacao?.inicio || payload?.cobertura?.inicio || ''
  });
  const existe = fila.some(item => JSON.stringify({
    acao: item?.payload?.acao,
    id: item?.payload?.situacao?.id || item?.payload?.cobertura?.id || item?.payload?.colaborador?.id || item?.payload?.colaboradorId || item?.payload?.codigo || item?.payload?.posto?.codigo || '',
...
function renderizarPainelCoberturasAdmin() {
  ...
  campo.innerHTML = itens.length ? itens.map(item => {
    ...
    return `<div class="linha-resumo-admin ${periodoContemData(item, hoje) ? 'cobertura-vigente' : ''}">
      <div><strong>${escaparHTML(destino?.nome || item.nomeDestino || 'Substituto')} → ${escaparHTML(origem?.nome || item.nomeOrigem || 'Colaborador')}</strong><small>${formatarDataBR(item.inicio)} a ${formatarDataBR(item.fim)} • ${estado}</small></div>
...
async function registrarCobertura() {
  ...
  if (!origem || !destino || origemId === destinoId) { toast('Selecione dois colaboradores diferentes.', 'erro'); return; }
  if (!inicio || !fim || fim < inicio) { toast('Informe um período válido.', 'erro'); return; }
  const confirmado = await confirmarAcaoApp({ titulo: 'Registrar cobertura', mensagem: `${destino.nome} cobrirá ${origem.nome}\n\nPeríodo: ${formatarDataBR(inicio)} a ${formatarDataBR(fim)}.\nA escala fixa de ${origem.nome} não será apagada.`, confirmarTexto: 'Registrar cobertura', tipo: 'aviso' });
  if (!confirmado) return;
  const item = { id: criarId('cob', `${origemId}-${destinoId}`), origemId, nomeOrigem: origem.nome, destinoId, nomeDestino: destino.nome, inicio, fim, ativo: true };
...
function editarEscalaSabadoDoPreview() {
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.5.0'}`;
