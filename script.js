...
const ACOES_ADMIN_DADOS = new Set([
  'salvarColaborador', 'excluirColaborador', 'retirarColaborador', 'preencherVaga', 'salvarPosto', 'excluirPosto',
  ...
  conteudo.innerHTML = `
    <div class="form-preencher-vaga">
      ...
      <div class="modal-campo-bloco">
        ...
        <select id="selectVagaDestinoRealocar">${vagas.length ? vagas.map((item) => `<option value="${escaparHTML(item.id)}" ${vaga?.id === item.id ? 'selected' : ''}>A DEFINIR • ${escaparHTML(item.turno)} • ${escaparHTML(resumoVagaAberta(item))}</option>`).join('') : '<option value="">Nenhuma vaga A DEFINIR neste turno</option>'}</select>
...
async function realocarColaboradorPeloModal() {
  ...
  const confirmado = await confirmarAcaoApp({
    ...
    mensagem: `${pessoa.nome} assumirá a vaga selecionada (${vagaDestino.turno}).\n\nNova posição:\n${resumoVagaAberta(vagaDestino)}\n\nPosição atual de ${pessoa.nome}:\n${resumoEscalaPessoa(pessoa)}\n\nA posição atual NÃO será apagada. Ela será transformada em uma nova vaga A DEFINIR para ser preenchida depois.`,
  ...
  conteudo.innerHTML = `
    <div class="form-preencher-vaga">
      <div class="modal-campo-bloco">
        ...
        <select id="selectVagaAberta">${vagas.map((vaga) => `<option value="${escaparHTML(vaga.id)}" ${vaga.id === selecionada.id ? 'selected' : ''}>A DEFINIR • ${escaparHTML(vaga.turno)} • ${escaparHTML(resumoVagaAberta(vaga))}</option>`).join('')}</select>
      </div>
      <div class="resumo-vaga"><strong>A DEFINIR • ${escaparHTML(selecionada.turno)}</strong><span>${escaparHTML(resumoVagaAberta(selecionada))}</span></div>
      <div class="modal-campo-bloco">
...
async function preencherVagaPeloModal() {
  ...
  const confirmado = await confirmarAcaoApp({
    ...
    mensagem: `Transferir a escala desta vaga para ${pessoa.nome}?\n\nTurno: ${vaga.turno}\n${resumoVagaAberta(vaga)}\n\nA vaga A DEFINIR desaparecerá e ${pessoa.nome} assumirá os mesmos postos dos 31 dias.`,
  ...
  try {
    ...
    msgAdmin(`${pessoa.nome} assumiu a vaga A DEFINIR de ${vaga.turno}.`);
...
async function executarRetiradaEquipe(pessoa) {
  ...
  const vaga = {
    id: vagaId, nome: 'A DEFINIR', registro: '', turno: turnoCanonico(pessoa.turno), ativo: true,
  ...
  atualizarInterfaceCompleta({ preservarAdmin: true });
  fecharModalCrudAdmin();
  msgAdmin(`${pessoa.nome} foi retirado da equipe. A escala foi preservada como A DEFINIR.`);
  toast('Colaborador retirado. Vaga A DEFINIR criada.', 'ok', 5000);
...
function resumoVagaAberta(pessoa) {
  ...
  return `Referências: ${primeiros.join(' | ')}${postos.length > 3 ? ` +${postos.length - 3}` : ''}`;
}
...
async function processarFilaOffline() {
  ...
  for (const item of fila) {
    try {
      ...
      if (['salvarColaborador','excluirColaborador','retirarColaborador','preencherVaga','salvarPosto','excluirPosto','salvarEscala','salvarSituacao','removerSituacao','salvarCobertura','removerCobertura'].includes(String(item?.payload?.acao || ''))) {
  ...
  $('resultadoConteudo').innerHTML = `
    <div class="lista-cards">
      ...
      ${pessoas.length ? pessoas.map((pessoa) => {
        ...
        const subtitulo = [pessoa.turno, ehVagaAberta(pessoa) ? 'Vaga aberta' : '', diaEscala.descricaoSituacao, diaEscala.cobrindoTexto, diaEscala.coberturaNome ? `Cobertura: ${diaEscala.coberturaNome}` : ''].filter(Boolean).join(' • ');
        const descricao = diaEscala.cobrindo
          ? diaEscala.origemCoberturaTexto
          : (diaEscala.situacao && diaEscala.posto ? `Posto de referência: ${diaEscala.posto} — ${diaEscala.descricaoPosto}` : diaEscala.descricaoPosto);
        return card({ titulo: pessoa.nome, badge: diaEscala.posto || '—', descricao, subtitulo, hoje: dia === destaque, situacao: Boolean(diaEscala.situacao), vaga: ehVagaAberta(pessoa) });
  ...
  pessoasPorTurno(turno).forEach((pessoa) => {
    pessoa.dias.slice(0, diasNoMes()).forEach((valorDia, indice) => {
      ...
      if (diaEscala.posto === postoBusca) itens.push(card({
        ...
        subtitulo: [pessoa.turno, diaEscala.descricaoSituacao, diaEscala.cobrindoTexto, diaEscala.coberturaNome ? `Cobertura: ${diaEscala.coberturaNome}` : ''].filter(Boolean).join(' • '),
        hoje: indice + 1 === destaque,
        situacao: Boolean(diaEscala.situacao),
...
function preencherSelectsAdmin() {
  ...
  const opcoesEscala = pessoasEscala.map((pessoa) => `<option value="${escaparHTML(pessoa.id)}">${escaparHTML(pessoa.nome)} • ${escaparHTML(pessoa.turno)}${ehVagaAberta(pessoa) ? ' • VAGA ABERTA' : ''}</option>`).join('');
...
function garantirPayloadAdminNaFila(payload) {
  ...
  const chave = JSON.stringify({
    acao: payload?.acao,
    id: payload?.situacao?.id || payload?.cobertura?.id || payload?.vagaId || payload?.vagaDestinoId || payload?.novaVagaId || payload?.colaborador?.id || payload?.colaboradorId || payload?.codigo || payload?.posto?.codigo || '',
    inicio: payload?.situacao?.inicio || payload?.cobertura?.inicio || ''
  });
  const existe = fila.some(item => JSON.stringify({
    acao: item?.payload?.acao,
    id: item?.payload?.situacao?.id || item?.payload?.cobertura?.id || item?.payload?.vagaId || item?.payload?.vagaDestinoId || item?.payload?.novaVagaId || item?.payload?.colaborador?.id || item?.payload?.colaboradorId || item?.payload?.codigo || item?.payload?.posto?.codigo || '',
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.5.6'}`;
