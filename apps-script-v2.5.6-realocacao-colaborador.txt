...
function retirarColaborador_(dados) {
  ...
  upsertPorChave_(NOMES_ABAS.colaboradores, CABECALHOS.colaboradores.length, 1, vagaId, [vagaId, 'A DEFINIR', '', pessoa.turno, true, agora]);
  salvarLinhaEscala_(CHAVE_ESCALA_FIXA, { id: vagaId, nome: 'A DEFINIR', registro: '', turno: pessoa.turno, ativo: true, dias });
  registrarHistorico_('RETIRAR DA EQUIPE', `${pessoa.nome} • ${pessoa.turno} → A DEFINIR (${vagaId}).`, 'Administrador');
  return { sucesso: true, vagaId, mensagem: 'Colaborador retirado e escala preservada como A DEFINIR.' };
}
...
function preencherVaga_(dados) {
  ...
  const vaga = { id: String(vagaCol[0] || ''), nome: String(vagaCol[1] || ''), turno: turnoCanonico_(vagaCol[3] || '') };
  ...
  registrarHistorico_('PREENCHER VAGA', `${vagaId} → ${pessoa.nome} • ${pessoa.turno}.`, 'Administrador');
...
function realocarColaborador_(dados) {
  ...
  upsertPorChave_(NOMES_ABAS.colaboradores, CABECALHOS.colaboradores.length, 1, novaVagaId, [novaVagaId, 'A DEFINIR', '', pessoa.turno, true, agora]);
  salvarLinhaEscala_(CHAVE_ESCALA_FIXA, { id: novaVagaId, nome: 'A DEFINIR', registro: '', turno: pessoa.turno, ativo: true, dias: diasAtuais });

  registrarHistorico_('REALOCAR COLABORADOR', `${pessoa.nome} • ${pessoa.turno} assumiu ${vagaDestinoId}; posição anterior preservada como ${novaVagaId}.`, 'Administrador');
  return { sucesso: true, novaVagaId, mensagem: `${pessoa.nome} realocado e posição anterior preservada como A DEFINIR.` };
}
